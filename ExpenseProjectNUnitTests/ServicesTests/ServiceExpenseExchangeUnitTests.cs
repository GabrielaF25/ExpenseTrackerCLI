...
public class ServiceExpenseExchangeUnitTests
{
    ...
    private IExpenseExchangeService _service;
    ...
    [Test]
    public void ConvertExpenseCurrencyFromRon_WhenCalled_ChangeExpenseCurrency()
    {
        //Arrange
        ...
        _mockServices.Setup(x => x.GetExpenseById(1)).Returns(expense);
        _mockProvider.Setup(x => x.GetValue(currencyType)).Returns(5.07m);
        _mockServices.Setup(x => x.Update(expense)).Returns(ResultResponse.Success());
        _mockdate.Setup(x => x.SetDateTimeNow()).Returns(DateTime.Now);

        //Act
        var result = _service.ConvertExpenseCurrencyFromRon(1, currencyType);
    ...
    public void ConvertExpenseCurrencyFromRon_WhenCalled_ShoulReturnFailedMessageUpdate()
    {
        //Arrange
        var currencyType = CurrencyType.Euro;

        _mockServices.Setup(x => x.GetExpenseById(1)).Returns(new Expense());
        _mockProvider.Setup(x => x.GetValue(currencyType)).Returns(5.07m);
        _mockServices.Setup(x => x.Update(It.IsAny<Expense>())).Returns(ResultResponse.Failure("Sorry! Something went wrong!"));
        _mockdate.Setup(x => x.SetDateTimeNow()).Returns(DateTime.Now);
    ...
    [Test]
    public void ConvertExpenseCurrencyFromRon_WhenCalled_ReturnFailedMessage()
    {
        //Arrange
        var currencyType = CurrencyType.Euro;

        _mockServices.Setup(x => x.GetExpenseById(1)).Returns((Expense?)null);
        ...
        _mockServices.Verify(x => x.Update(It.IsAny<Expense>()), Times.Never);
    }
    #endregion
    ...
    [Test]
    public void ConvertExpenseCurrencyToRon_WhenCalled_ChangeExpenseCurrency()
    {
        //Arrange
        var expense = new Expense()
        {
        ...
        //Act
        var result = _service.ConvertExpenseCurrencyToRon(1);
    ...
    [Test]
    public void ConvertExpenseCurrencyToRon_WhenCalled_ShoulReturnFailedMessageUpdate()
    {
        //Arrange
        _mockServices.Setup(x => x.GetExpenseById(1)).Returns(new Expense());
    ...
    [Test]
    public void ConvertExpenseCurrencyToRon_WhenCalled_ReturnFailedMessage()
    {
        //Arrange
        _mockServices.Setup(x => x.GetExpenseById(1)).Returns((Expense?)null);
